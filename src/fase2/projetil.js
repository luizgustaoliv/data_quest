    ...
    try {
        ...
        class TutorialScene extends Phaser.Scene {
            ...
            preload() {
                ...
                this.load.image('fundo', 'assets/fase2/minigame-projetil/backgroundtutorial.jpeg');
            ...
            create() {
                ...
                const tutorialText = this.add.text(
                    this.scale.width / 2, 
                    100, 
                    'TUTORIAL: Conceitos de Movimento', 
                ...
                const continueButton = this.add.text(
                    ...
                    { fontSize: '24px', fill: '#fff', backgroundColor: '#1a6e1a', padding: { x: 20, y: 10 } }
                ...
                this.physicsInfoText = this.add.text(10, 10, '', { fontSize: '16px', fill: '#fff' });
            ...
            lancarChave() {
                ...
                this.animationInterval = setInterval(() => {
                    ...
                    this.physicsInfoText.setText(
                        ...
                        `MU: X = ${this.chave.x.toFixed(2)} | Velocidade X = ${velocidadeX.toFixed(2)} (constante)\n` +
                        `MUV: Y = ${this.chave.y.toFixed(2)} | Velocidade Y = ${velocidadeY.toFixed(2)} | Aceleração Y = ${aceleracaoY.toFixed(2)}`
                    );
        ...
        class GameScene extends Phaser.Scene {
            ...
            create() {
                ...
                const avisoObstaculos = this.add.text(
                    this.scale.width / 2,
                    70,
                    'Cuidado com os vírus em movimento!',
                    { fontSize: '18px', fill: '#ff0000', stroke: '#000000', strokeThickness: 2 }
                ).setOrigin(0.5);
            ...
            criarCadeados() {
                for (let i = 0; i < this.totalDados; i++) {
                    ...
                    const x = Phaser.Math.Between(600, Math.min(1000, this.scale.width - 100));
                    const y = Phaser.Math.Between(200, Math.min(600, this.scale.height - 100));
                ...
                const areaAlvos = this.add.text(
                    ...
                    150,
                    'Área de Alvos',
                    { fontSize: '18px', fill: '#ffff00', stroke: '#000000', strokeThickness: 2 }
                );
            ...
            colisaoObstaculo(projetil, obstaculo) {
                ...
                // Cria efeito visual
                const explosao = this.add.circle(projetil.x, projetil.y, 30, 0xff0000, 0.8);
                this.tweens.add({
                    targets: explosao,
                    alpha: 0,
                    scale: 2,
                ...
                const mensagemFalha = this.add.text(
                    this.scale.width / 2,
                    this.scale.height / 2,
                    'Você atingiu um obstáculo!\nReiniciando...',
                    { fontSize: '32px', fill: '#ff0000', stroke: '#000000', strokeThickness: 4, align: 'center' }
                ).setOrigin(0.5);
            ...
            update() {
                if (this.projetilAtivo) {
                    ...
                    this.physicsInfoText.setText(
                        `MU: Posição X: ${this.projetilAtivo.x.toFixed(2)}, Velocidade X: ${this.projetilAtivo.body.velocity.x.toFixed(2)}\n` +
                        `MUV: Posição Y: ${this.projetilAtivo.y.toFixed(2)}, Velocidade Y: ${this.projetilAtivo.body.velocity.y.toFixed(2)}, Aceleração Y: 250`
                    );
                    
                    if (
                        this.projetilAtivo.x < 0 || this.projetilAtivo.x > this.scale.width ||
                        this.projetilAtivo.y < 0 || this.projetilAtivo.y > this.scale.height
                    ) {
