...
function construirPagina() {
  ...
  const gameContainer =
    document.getElementById("game-container") || document.createElement("div");
...
function createFaseCard(fase) {
      ...
      setTimeout(() => {
        ...
        if (fase.number === 1) {
          ...
          if (window.fase1Initialized) {
            console.log(
              "Fase1.js já carregado, iniciando a fase imediatamente."
            );
            if (!window.Phaser) {
            ...
            scriptElement.onerror = () => {
              ...
              alert(
                "Ocorreu um erro ao carregar a Fase 1. Por favor, tente novamente."
              );
  ...
  statusText.textContent =
    fase.status === "available" ? "DISPONÍVEL" : "BLOQUEADA";
...
function iniciarFase1() {
  ...
  document
    .querySelectorAll(
      "#logo-container, #fase-container, #particles-container-front, #particles-container-back"
    )
    .forEach((el) => {
    ...
    const tryLoadScript = (paths, index) => {
      ...
      fase1Script.onload = function () {
        ...
        if (typeof window.startGame !== "function") {
          console.log(
            "Script carregado, mas função startGame não encontrada. Tentando próximo caminho..."
          );
      ...
      fase1Script.onerror = function () {
        console.error(
          `Erro ao carregar fase1.js de ${paths[index]}. Tentando próximo caminho...`
        );
        tryLoadScript(paths, index + 1);
      };
...
function createParticleLayer(containerId, count, options) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    particle.className = `particle ${options.blur ? "blur" : ""} ${
      options.glow ? "glow" : ""
    }`;
    ...
    const speed =
      Math.random() * (options.speedMax - options.speedMin) + options.speedMin;

    // Opacidade aleatória
    const opacity =
      Math.random() * (options.opacityMax - options.opacityMin) +
      options.opacityMin;
  ...
  if (containerId === "particles-container-front") {
    document.addEventListener("mousemove", (e) => {
      ...
      if (
        !window.lastParticleTime ||
        Date.now() - window.lastParticleTime > 100
      ) {
...
function createMouseParticle(container, x, y) {
  ...
  setTimeout(() => {
    particle.style.transform = `translate(${Math.random() * 50 - 25}px, ${
      Math.random() * 50 - 25
    }px)`;
